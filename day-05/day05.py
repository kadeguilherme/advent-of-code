...
with open("day05.txt", 'r') as arq:
    ...
    for instruc in instrucs:
        quant, stack1, stack2 = int(instruc.split(' ')[1]), int(instruc.split(
            ' ')[3]), int(instruc.split(' ')[-1])
        create_to_remove = stacks[stack1][-quant:]
        # removendondo as create_to_remove da stacks autal
        stacks[stack1] = stacks[stack1][:-quant]
        for i in create_to_remove:
            stacks[stack2].append(i)
